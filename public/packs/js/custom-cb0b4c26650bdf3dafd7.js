...
/* WEBPACK VAR INJECTION */(function($) {$(document).on('turbolinks:load', function () {
  $('body').on('select2:select', function (e) {
    console.log($('.color').find(':selected').text());
    var colors = new Map();
    $('.color').find(':selected').each(function (index) {
      console.log(index + ": " + $(this).text());

      if (colors.has($(this).text())) {
        colors.set($(this).text(), colors.get($(this).text()) + 1);
      } else {
        colors.set($(this).text(), 1);
      }
    });
    console.log(colors);
...
/******/ });
//# sourceMappingURL=custom-cb0b4c26650bdf3dafd7.js.map